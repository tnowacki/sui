...
// Generated by `CallArg::Withdrawal { type: TypeInput, location: address, limit: Enum<u256> }`
...
// - If the limit is specified, it is checked against all other `Withdrawals` with
...
public struct Withdrawal<phantom T: store> has drop {
    // TODO I need a name here that isn't account. Or is account fine?
...
public fun withdrawal_split<T: store>(
    ...
    assert!(withdrawal.limit >= value);
    withdrawal.limit = withdrawal.limit - value;
    Withdrawal { location: withdrawal.location, limit: value }
}

public fun withdrawal_(join|merge)<T: store>(
    withdrawal: &mut Withdrawal<T>,
    other: Withdrawal<T>,
): u256 {
    assert!(withdrawal.location == other.location);
    let old_limit = withdrawal.limit;
    withdrawal.limit = old_limit + other.limit;
    old_limit
}
...
// TODO lock and seize caps. Lock must be top level
...
// Generated by `CallArg::Seize { type: TypeInput, location: address, limit: Option<u256> }`
