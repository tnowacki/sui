...
async fn test_get_latest_parent_entry() {
    ...
    let update_version = SequenceNumber::lamport_increment([seq1, seq2, effects.gas_object().unwrap().1.unwrap().0.1]);
    ...
    let delete_version = SequenceNumber::lamport_increment([obj_ref.1, effects.gas_object().unwrap().1.unwrap().0.1]);
...
async fn test_transfer_sui_no_amount() {
    ...
    assert!(gas_ref.1 < effects.gas_object().unwrap().1.unwrap().0.1);
    assert_eq!(effects.gas_object().unwrap().1.unwrap().1, Owner::AddressOwner(recipient));
    let new_balance =
...
async fn test_transfer_sui_with_amount() {
    ...
    assert!(gas_ref.1 < effects.gas_object().unwrap().1.unwrap().0.1);
    assert_eq!(effects.gas_object().unwrap().1.unwrap().1, Owner::AddressOwner(sender));
    let new_balance =
        sui_types::gas::get_gas_balance(&authority_state.get_object(&gas_object_id).await.unwrap())
            .unwrap();
    assert_eq!(
...
async fn test_clear_cache_reverts_wrap_move_call() {
    ...
    let wrap_txn = to_sender_signed_transaction(
        TransactionData::new_move_call(
            ...
            ident_str!("wrap").to_owned(),
            vec![],
            create_effects.gas_object().unwrap().1.unwrap().0,
    ...
    assert_eq!(gas.version(), create_effects.gas_object().unwrap().1.unwrap().0.1);
}
...
async fn test_clear_cache_reverts_unwrap_move_call() {
    ...
    let unwrap_txn = to_sender_signed_transaction(
        TransactionData::new_move_call(
            ...
            vec![],
            wrap_effects.gas_object().unwrap().1.unwrap().0,
    ...
    // The gas is uncharged
    let gas = cache.get_object(&gas_object_id).unwrap();
    assert_eq!(gas.version(), wrap_effects.gas_object().unwrap().1.unwrap().0.1);
}
...
async fn create_and_retrieve_df_info(function: &IdentStr) -> (SuiAddress, Vec<DynamicFieldInfo>) {
    ...
    let add_txn = to_sender_signed_transaction(
        TransactionData::new_move_call(
            ...
            function.to_owned(),
            vec![],
            create_inner_effects.gas_object().unwrap().1.unwrap().0,
            vec![
...
async fn test_clear_cache_reverts_removed_ofield() {
    ...
    let remove_ofield_txn = to_sender_signed_transaction(
        TransactionData::new_move_call(
            ...
            vec![],
            add_effects.gas_object().unwrap().1.unwrap().0,
...
async fn test_gas_smashing() {
    ...
    async fn run_and_check(
        ...
        assert_eq!(gas_coin_ids[0], effects.gas_object().unwrap().0);
...
async fn test_publish_transitive_dependencies_ok() {
    ...
    let gas_ref = txn_effects.gas_object().unwrap().1.unwrap().0;
