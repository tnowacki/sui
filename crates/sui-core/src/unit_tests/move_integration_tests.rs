...
async fn test_object_wrapping_unwrapping() {
    ...
    let wrapped_version =
        SequenceNumber::lamport_increment([child_object_ref.1, effects.gas_object().unwrap().1.unwrap().0.1]);
    ...
    let unwrapped_version =
        SequenceNumber::lamport_increment([parent_object_ref.1, effects.gas_object().unwrap().1.unwrap().0.1]);
    ...
    let rewrap_version = SequenceNumber::lamport_increment([
        parent_object_ref.1,
        child_object_ref.1,
        effects.gas_object().unwrap().1.unwrap().0.1,
    ]);
