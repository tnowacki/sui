---
title: Coin Flip
description: Learn Sui through a coin flip app that covers the full end-to-end flow of building a Move module and connecting it to a React Sui app.
...
- **One-time witnesses:** The guide teaches you how to use [one-time witnesses](/concepts/sui-move-concepts.mdx) to ensure only a single instance of the `HouseData` object ever exists.
...
- [`@mysten/dapp`](https://sdk.mystenlabs.com/dapp-kit/create-dapp), used within this project to quickly scaffold a React-based Sui app.
...
To use a [programmable transaction block](/guides/developer/transactions/ptbs/prog-txn-blocks) (PTB) in Sui, create a `Transaction`. To initiate a Move call, you must know the global identifier of a public function in your smart contract. The global identifier usually takes the following form:
