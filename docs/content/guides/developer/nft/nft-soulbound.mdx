...
description: An example using Move struct abilities and the Sui Framework's `transfer` module to make a NFT soulbound (non-transferable).
...
You can also create custom transfer logic for NFTs on Sui. The `transfer::transfer` function has custom rules performed by the Move bytecode verifier that ensures that the transferred objects are defined in the module where transfer is invoked. While removing the `store` ability from a struct definition makes `transfer::public_transfer` unusable, `transfer::transfer` can still be used as long as you use it in the module that defined that object's type. This allows the module owner to provide custom transfer logic for their soulbound NFTs. 
