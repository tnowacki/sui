...
1. [**Nodes**](/guides/operator/index.mdx) that run the network's binaries and participate in network activity such as consensus. Nodes provide compute and [storage](/concepts/sui-architecture/sui-storage.mdx) resources to the network.

1. The [**consensus mechanism**](/concepts/sui-architecture/consensus.mdx) used to validate transactions.

1. [**Transactions**](/guides/developer/transactions/txn-overview.mdx) themselves, which on Sui reflect on-chain operations such as creating objects or sending assets.
...
Learn more about [nodes on Sui](/guides/operator/index.mdx).
...
Learn more about the [Sui Object Model](/guides/developer/objects/object-model.mdx) or [Object Ownership](/guides/developer/objects/object-ownership).

## Move

Move is an open source programming language that is used for all activity on Sui, such as creating or trading NFTs, using apps, and all other transaction-based events. It is used for writing safe packages that can create and manipulate objects on-chain.
