...
description: Learn how to use Display V2 format strings to render Move object values into human-readable strings, JSON, or encoded representations.
keywords: [ display, template, format string, move, rendering ]
---

Display V2 is a template language for rendering Move object values into human-readable strings, JSON, or encoded representations. A `Display` object defines a set of key-value pairs where both keys and values are **format strings**.
